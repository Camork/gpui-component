...
impl Element for FlameCanvas {
    ...
    fn prepaint(
        ...
        // Tell the scrollbar how tall the viewport is, so its thumb drag clamp
        // is exact. content_h is refreshed by the view every render.
        let scroll = self.view.read(cx).scroll.0.clone();
        scroll.borrow_mut().viewport_h = bounds.size.height.as_f32();
    ...
    fn paint(
        ...
        // ── phase 1: read the view into owned values ──────────────────────
        {
            ...
            for (ti, track) in view.flat.tracks.iter().enumerate() {
                ...
                let rh = (track.max_depth + 1) as f32 * style.frame_row_height;
                ...
                let last_row = if rows_y + rh > height {
                    (((height - rows_y) / style.frame_row_height).ceil() as usize).min(track.max_depth + 1)
                } else {
                    track.max_depth + 1
                };
                ...
                for (fi, f) in track.frames.iter().enumerate() {
                    ...
                    let rect = Bounds::from_corners(
                        point(origin.x + px(x0), origin.y + px(frame_y)),
                        point(
                            origin.x + px(x1),
                    ...
                    let matched = view.frame_matched(&f.name);
                    if !matched {
                    ...
                    if x1 - x0 >= LABEL_MIN_WIDTH && matched {
                        let line = shape_label(
                            window,
                            truncate_label(&f.name, x1 - x0 - 6.0),
                            style.frame_font_size,
                            style.frame_text_color,
                        );
                        sink.texts.push((
                            line,
                            point(origin.x + px(x0) + px(3.0), origin.y + px(frame_y)),
...
impl FlameCanvas {
    ...
    fn register_interactions(
        ...
        window.on_mouse_event(move |e: &MouseDownEvent, phase, window, cx| {
            ...
            // letting the sibling Scrollbar component handle mouse interaction.
            if cx_px >= width - 12.0 && content_h > height + 1.0 {
                return;
            }

            let scroll_y = down_view.read(cx).scroll_y();
            let cy = local.y.as_f32() + scroll_y;
            let press_time = down_view
                .read(cx)
                .viewport
                .x_to_time(cx_px as f64, width as f64);
            ...
            if e.click_count >= 2 {
                down_view.update(cx, |view, cx| {
                    if let Some(ti) = view.header_at(cy) {
                        ...
                        if let Some(track) = view.flat.tracks.get(fid.track) {
...
fn collect_track_header(
    ...
    let name_line = shape_label(window, track.name.clone().into(), style.header_font_size, sink.fg);
    sink.texts.push((
        name_line.clone(),
