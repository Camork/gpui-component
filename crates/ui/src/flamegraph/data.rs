...
fn layout_track(track: &Track) -> TrackLayout {
    let mut rows: Vec<Vec<FlatFrame>> = Vec::new();
    fn visit(node: &FlameFrame, depth: usize, rows: &mut Vec<Vec<FlatFrame>>) {
        if depth >= rows.len() {
            rows.resize_with(depth + 1, Vec::new);
        }
        let start = secs(&node.start);
        rows[depth].push(FlatFrame {
    ...
    for row in &mut rows {
        row.sort_by(|a, b| a.abs_start.partial_cmp(&b.abs_start).unwrap_or(std::cmp::Ordering::Equal));
    }

    let max_depth = rows.len().saturating_sub(1);
    TrackLayout {
